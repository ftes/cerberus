defmodule Cerberus.PhoenixTest.StaticTest do
  ...
  describe "click_link/2" do
    ...
    test "raises error if trying to submit via `data-` attributes but incomplete", %{conn: conn} do
      ...
      |> then(fn session ->
        assert Driver.current_path(session) == "/users/2"
      end)
    end
  ...
  describe "click_button/2" do
    ...
    test "raises an error when there are no buttons on page", %{conn: conn} do
      msg = ~r/no button matched locator/
  ...
  describe "within/3" do
    ...
    test "raises when data is not in scoped HTML", %{conn: conn} do
      assert_raise AssertionError, ~r/no form field matched locator/, fn ->
        conn
        |> visit("/page/index")
        |> within("#email-form", fn session ->
  ...
  describe "fill_in/4" do
    ...
    test "raises an error when element can't be found with label", %{conn: conn} do
      msg = ~r/no form field matched locator/
  ...
  describe "submit/1" do
    ...
    test "raises an error if there's no active form", %{conn: conn} do
      msg = ~r/submit\/1 requires an active form/
  ...
  describe "open_browser" do
    test "opens the browser", %{conn: conn} do
      ...
      |> open_browser(fn path ->
        ...
        assert content =~
                 ~r[<link phx-track-static="" rel="stylesheet" href="file:.*priv\/static\/assets\/app\.css"\/>]
  ...
  describe "unwrap" do
    ...
    test "provides an escape hatch that gives access to the underlying conn", %{conn: conn} do
      ...
      |> visit("/page/index")
      |> unwrap(fn conn ->
        Plug.Conn.assign(conn, :unwrap_test, "hello")
      end)
      |> then(fn %{conn: conn} ->
        assert conn.assigns.unwrap_test == "hello"
      end)
    end
    ...
    test "follows redirects after unwrap action", %{conn: conn} do
      ...
      |> visit("/page/page_2")
      |> unwrap(fn conn ->
        redirected = Phoenix.ConnTest.post(conn, "/phoenix_test/page/redirect_to_static", %{})
        Phoenix.ConnTest.get(redirected, Phoenix.ConnTest.redirected_to(redirected))
      end)
      |> assert_has("h1", text: "Main page")
    end
  end
