defmodule Cerberus.PhoenixTest.LiveTest do
  use ExUnit.Case, async: true

  import Cerberus.TestSupport.PhoenixTest.Legacy

  alias Cerberus.TestSupport.PhoenixTest.ActiveForm
  ...
  describe "click_link/2" do
    ...
    test "raises an error when link element can't be found with given text", %{conn: conn} do
      assert_raise AssertionError, ~r/no link matched locator|No link/, fn ->
        conn
        |> visit("/live/index")
        |> click_link("No link")
      end
  ...
  describe "click_button" do
    ...
    test "does not remove active form if button isn't form's submit button", %{conn: conn} do
      ...
      assert ActiveForm.active?(session.form_data.active_form)
    end
    ...
    test "resets active form if it is form's submit button", %{conn: conn} do
      ...
      refute ActiveForm.active?(session.form_data.active_form)
    end
    ...
    test "Raises an error if button with type 'button' inside form doesn't have valid phx-click", %{conn: conn} do
      msg = ~r/no button matched locator|Non submit button/
    ...
    test "raises an error if button is disabled", %{conn: conn} do
      msg = ~r/no button matched locator|Disabled button 1/
    ...
    test "raises an error if form doesn't have a `phx-submit` or `action`", %{conn: conn} do
      msg = ~r/`phx-submit` or `action` defined/
    ...
    test "raises an error when there are no buttons on page", %{conn: conn} do
      assert_raise AssertionError, ~r/no button matched locator|Show tab/, fn ->
        conn
        |> visit("/live/page_2")
        |> click_button("Show tab")
      end
    ...
    test "raises an error if button is not part of form and has no phx-submit", %{conn: conn} do
      msg = ~r/no button matched locator|Actionless Button/
    ...
    test "raises an error if active form but can't find button", %{conn: conn} do
      msg = ~r/no button matched locator|No button/
  ...
  describe "within/3" do
    ...
    test "raises when data is not in scoped HTML", %{conn: conn} do
      assert_raise AssertionError, ~r/no form field matched locator|User Name/, fn ->
        conn
        |> visit("/live/index")
        |> within("#email-form", fn session ->
  ...
  describe "fill_in/4" do
    ...
    test "raises an error when element can't be found with label", %{conn: conn} do
      msg = ~r/no form field matched locator|Non-existent Email Label/
    ...
    test "raises an error when label is found but no corresponding input is found", %{conn: conn} do
      msg = ~r/no form field matched locator|Email \(no input\)/
  ...
  describe "upload/4" do
    ...
    test "upload (without other form actions) does not work with submit (matches browser behavior)", %{conn: conn} do
      ...
      assert_raise AssertionError, ~r/requires an active form/, fn ->
        submit(session)
      end
    end
    ...
    test "throws a nice error message for the `:not_accepted` error", %{conn: conn} do
      ...
      assert_raise AssertionError, ~r/Unsupported file type/, fn ->
    ...
    test "throws a nice error message for the `:too_many_files` error", %{conn: conn} do
      ...
      assert_raise AssertionError, ~r/Too many files uploaded/, fn ->
        session
        |> upload("Avatar", "test/files/elixir.jpg")
        |> upload("Avatar", "test/files/elixir.jpg")
      end
    ...
    test "throws a nice error message for the `:too_large` error", %{conn: conn} do
      session =
        conn
        |> visit("/live/index")
        |> within("#tiny-upload-form", & &1)

      assert_raise AssertionError, ~r/File too large/, fn ->
        upload(session, "Tiny", "test/files/large.jpg")
      end
    end
  ...
  describe "submit/1" do
    ...
    test "raises an error if there's no active form", %{conn: conn} do
      message = ~r/requires an active form/
  ...
  describe "shared form helpers behavior" do
    ...
    test "raises an error if multiple forms have phx-trigger-action", %{conn: conn} do
      assert_raise AssertionError, ~r/Found multiple forms/, fn ->
        conn
        |> visit("/live/index")
    ...
    test "raises an error if field doesn't have a `name` attribute", %{conn: conn} do
      assert_raise AssertionError, ~r/no form field matched locator|No Name Attribute/, fn ->
        conn
        |> visit("/live/index")
        |> within("#invalid-form", fn session ->
  ...
  describe "assert_has/3 with timeout" do
    test "uses default timeout for live assertions", %{conn: conn} do
      conn
    ...
    test "timeout waits for changes to LiveView as a result of info messages", %{conn: conn} do
      ...
      |> assert_has("h2", text: "I've been changed!", timeout: 250)
    end
    ...
    test "timeout handles redirects", %{conn: conn} do
      conn
      |> visit("/live/async_page")
      |> click_button_with_redirect_fallback("Async redirect!")
  ...
  defp click_button_with_redirect_fallback(session, text) do
    click_button(session, text)
  catch
    :exit, {:shutdown, {:redirect, %{to: to}}} ->
      visit(session, to)
  end
end
