defmodule Cerberus.PhoenixTestPlaywright.Upstream.AssertionsTest do
  ...
  defp assert_compat_error(expected, fun) when is_function(fun, 0) do
    ...
    error in [ArgumentError, AssertionError] ->
  ...
  describe "assert_has/3" do
    ...
    test "raises an error if label and value are found more than expected", %{conn: conn} do
      ...
      assert_raise AssertionError, ~r/Kingdoms.*Gondor|Gondor.*Kingdoms/, fn ->
    ...
    test "raises if user provides :text and :value options", %{conn: conn} do
      ...
      assert_compat_error(~r/Could not find|text.*value|value.*text/, fn ->
  ...
  describe "refute_has/3" do
    ...
    test "raises an error if label and value are found more/less than expected", %{conn: conn} do
      ...
      assert_raise AssertionError, ~r/input.*Kingdoms.*Gondor|input.*Gondor.*Kingdoms/, fn ->
