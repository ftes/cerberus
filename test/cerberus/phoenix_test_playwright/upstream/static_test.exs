defmodule Cerberus.PhoenixTestPlaywright.Upstream.StaticTest do
  use Cerberus.TestSupport.PhoenixTestPlaywright.Case, async: true
  ...
  defp assert_compat_error(expected, fun) when is_function(fun, 0) do
    ...
    error in [ArgumentError, ExUnit.AssertionError] ->
  ...
  describe "click_link/2" do
    ...
    @tag skip: "phoenix_test_playwright data-method link parity bug"
    ...
    @tag skip: "phoenix_test_playwright click_link ambiguity parity bug"
    ...
    test "raises an error when link element can't be found with given text", %{conn: conn} do
      assert_compat_error(~r/no link matched locator|Could not find element with selector/, fn ->
        conn
        |> visit("/page/index")
        |> click_link("No link")
      end)
  ...
  describe "click_button/2" do
    ...
    @tag skip: "phoenix_test_playwright hidden _method form submission parity bug"
    ...
    @tag skip: "phoenix_test_playwright browser redirect settle parity bug"
    ...
    @tag skip: "phoenix_test_playwright data-method button parity bug"
    test "handles form submission via `data-method` & `data-to` attributes", %{conn: conn} do
      conn
      |> visit("/page/index")
    ...
    test "does not remove active form if button isn't form's submit button", %{conn: conn} do
      ...
      assert ActiveForm.active?(session.active_form)
    end
    ...
    test "resets active form if it is form's submit button", %{conn: conn} do
      ...
      refute ActiveForm.active?(session.active_form)
    end
    ...
    test "raises an error when there are no buttons on page", %{conn: conn} do
      msg = ~r/no button matched locator|Could not find element with selector/

      assert_compat_error(msg, fn ->
        conn
        |> visit("/page/page_2")
  ...
  describe "within/3" do
    ...
    test "raises when data is not in scoped HTML", %{conn: conn} do
      assert_compat_error(~r/no form field matched locator|Could not find element.*"User Name"/, fn ->
        conn
        |> visit("/page/index")
        |> within("#email-form", fn session ->
  ...
  describe "fill_in/4" do
    ...
    @tag skip: "phoenix_test_playwright submit-without-button parity bug"
    ...
    @tag skip: "phoenix_test_playwright duplicate-label exact matching parity bug"
    ...
    test "raises an error when element can't be found with label", %{conn: conn} do
      msg = ~r/no form field matched locator|Could not find element.*"Non-existent Email Label"./
    ...
    test "raises an error when label is found but no corresponding input is found", %{conn: conn} do
      msg = ~r/no form field matched locator|Could not find element/
  ...
  describe "current_path" do
    test "it is set on visit", %{conn: conn} do
      ...
      assert Driver.current_path(session) == "/page/index"
    end

    test "it includes query string if available", %{conn: conn} do
      session = visit(conn, "/page/index?foo=bar")

      assert Driver.current_path(session) == "/page/index?foo=bar"
    end
    ...
    test "it is updated on href navigation", %{conn: conn} do
      ...
      assert Driver.current_path(session) == "/page/page_2?foo=bar"
    end
