defmodule Cerberus.Fixtures.PhoenixTest.IndexLive do
  @moduledoc false
  ...
  def render(assigns) do
    ...
    <.link navigate="/phoenix_test/live/page_2?details=true&foo=bar">Navigate link</.link>
    <.link patch="/phoenix_test/live/index?details=true&foo=bar">Patch link</.link>
    <.link href="/phoenix_test/page/index?details=true&foo=bar">Navigate to non-liveview</.link>

    <.link class="multiple_links" href="/phoenix_test/live/page_3">Multiple links</.link>
    <.link class="multiple_links" href="/phoenix_test/live/page_4">Multiple links</.link>

    <.link navigate="/phoenix_test/live/page_2?redirect_to=/phoenix_test/live/index">Navigate (and redirect back) link</.link>
    ...
    <form id="non-liveview-form" action="/phoenix_test/page/create_record" method="post">
    ...
    <form id="pre-rendered-data-non-liveview-form" action="/phoenix_test/page/create_record" method="post">
  ...
  defp handle_progress(:redirect_avatar, entry, socket) do
    if entry.done? do
      {:noreply, push_navigate(socket, to: "/phoenix_test/live/page_2", replace: true)}
    else
      {:noreply, socket}
    end
  end
  ...
  def handle_event("patch-and-trigger-form", _form_data, socket) do
    {:noreply,
     ...
     |> push_patch(to: "/phoenix_test/live/index/alias")}
  end

  def handle_event("redirect-and-trigger-form", _form_data, socket) do
    {:noreply,
     socket
     |> redirect(to: "/phoenix_test/live/page_2")
     |> assign(:redirect_and_trigger_submit, true)}
  end
  ...
  def handle_event("navigate-and-trigger-form", _form_data, socket) do
    {:noreply,
     socket
     |> push_navigate(to: "/phoenix_test/live/page_2")
     |> assign(:redirect_and_trigger_submit, true)}
  end
  ...
  def handle_event("change-redirect-form", _, socket) do
    {:noreply,
     ...
     |> push_navigate(to: "/phoenix_test/auth/live/page_2")}
  end
  ...
  def handle_event("push-navigate", _, socket) do
    {:noreply, push_navigate(socket, to: "/phoenix_test/live/page_2?foo=bar")}
  end

  def handle_event("push-patch", _, socket) do
    {:noreply, push_patch(socket, to: "/phoenix_test/live/index?foo=bar")}
  end
  ...
  def handle_event("hook_with_redirect_event", _params, socket) do
    {:noreply, push_navigate(socket, to: "/phoenix_test/live/page_2")}
  end
