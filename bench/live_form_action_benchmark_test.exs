results_path = System.get_env("CERBERUS_BENCH_RESULTS_PATH") || "tmp/live-form-action-benchmark.csv"

File.mkdir_p!(Path.dirname(results_path))
File.rm(results_path)

defmodule Cerberus.Bench.LiveFormActionBenchmark do
  @moduledoc false
...
defmodule Cerberus.LiveFormActionBenchmarkTest do
  ...
  test "live form action benchmark worker", %{conn: conn, worker: worker} do
...
ExUnit.after_suite(fn _results ->
  ...
  if File.exists?(results_path) do
    ...
    round_ms = Cerberus.Bench.LiveFormActionBenchmark.round_duration_ms(entries)

    IO.puts(Cerberus.Bench.LiveFormActionBenchmark.csv_header())
    IO.puts(Cerberus.Bench.LiveFormActionBenchmark.single_round_row(concurrency, round_ms))
