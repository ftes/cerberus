defmodule Cerberus.Browser.Install do
  ...
  @type binaries ::
          ...
          | %{
              required(:firefox_binary) => String.t()
            }
  ...
  @type versions ::
          ...
          | %{
              required(:firefox_version) => String.t()
            }
  ...
  defp parse_payload(:firefox, values) when is_map(values) do
    required_keys = ["firefox_binary", "firefox_version"]
  ...
  defp installer_script_path(browser) when browser in [:chrome, :firefox] do
    root = Mix.Project.deps_paths()[:cerberus] || local_project_root()
    candidate = Path.expand("bin/#{browser}.sh", root)
