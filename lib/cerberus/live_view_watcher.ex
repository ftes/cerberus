defmodule Cerberus.LiveViewWatcher do
  ...
  defp add_to_monitored_views(watched_views, live_view) do
    case watched_views[live_view.pid] do
      nil ->
        ...
        {:ok, Map.put(watched_views, live_view.pid, view)}
  ...
  defp monitor_view(live_view) do
    ...
    %{pid: live_view.pid, live_view_ref: live_view_ref}
  end
  ...
  defp remove_view(state, view_pid) do
    case state.views[view_pid] do
      nil -> state
      _view -> %{state | views: Map.delete(state.views, view_pid)}
    end
  end
end
