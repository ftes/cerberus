defmodule Cerberus.Driver.Browser.CdpPageProcess do
  ...
  alias Bibbidi.Transport.MintWS
  ...
  @type state :: %{
          ...
          pending: %{optional(pos_integer()) => pending_entry()}
        }
  ...
  def init(opts) do
    ...
    with {:ok, socket_url} <- fetch_page_socket_url(debugger_address, target_id, @list_retry_attempts),
         {:ok, socket_uri} <- parse_socket_uri(socket_url),
         {:ok, transport} <- connect_transport(socket_uri) do
      {:ok, %{target_id: target_id, transport: transport, next_id: 1, slow_mo_ms: slow_mo_ms, pending: %{}}}
    else
      {:error, reason} ->
  ...
  def handle_call({:evaluate, expression, timeout_ms}, from, state) do
    ...
    maybe_sleep_for_slow_mo(state.slow_mo_ms)

    case MintWS.send_message(state.transport, payload) do
      {:ok, transport} ->
        timer = Process.send_after(self(), {:command_timeout, id}, timeout_ms)
        pending = Map.put(state.pending, id, %{from: from, timer: timer})
        {:noreply, %{state | transport: transport, next_id: id + 1, pending: pending}}

      {:error, transport, reason} ->
        {:reply, {:error, inspect(reason), %{}}, %{state | transport: transport}}
    end
  end
  ...
  def handle_info(message, state) do
    case MintWS.handle_in(state.transport, message) do
      {:ok, transport, frames} ->
        {:noreply, Enum.reduce(frames, %{state | transport: transport}, &handle_frame/2)}
  ...
  def terminate(_reason, state) do
    _ = MintWS.close(state.transport)
    :ok
  end

  @spec handle_frame(term(), state()) :: state()
  defp handle_frame({:text, payload}, state) when is_binary(payload) do
    handle_decoded_frame(state, JSON.decode(payload))
  end

  defp handle_frame({:close, _code, reason}, state) do
    fail_pending(state, {:error, "cdp page socket disconnected", %{"reason" => inspect(reason)}})
  end

  defp handle_frame({:error, reason}, state) do
    fail_pending(state, {:error, "cdp page socket error", %{"reason" => inspect(reason)}})
  end

  defp handle_frame(_frame, state), do: state

  @spec fail_pending(state(), Types.bidi_response()) :: state()
  ...
  @spec parse_socket_uri(String.t()) :: {:ok, URI.t()} | {:error, String.t()}
  defp parse_socket_uri(socket_url) when is_binary(socket_url) do
    case URI.parse(socket_url) do
      %URI{scheme: scheme, host: host} = uri when scheme in ["ws", "wss"] and is_binary(host) ->
        {:ok, uri}

      _ ->
        {:error, "invalid cdp page websocket url"}
    end
  end

  @spec connect_transport(URI.t()) :: {:ok, struct()} | {:error, term()}
  defp connect_transport(socket_uri) do
    connect = &MintWS.connect/2
    connect.(socket_uri, [])
  end
end
