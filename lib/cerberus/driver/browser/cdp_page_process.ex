defmodule Cerberus.Driver.Browser.CdpPageProcess do
  ...
  @spec fetch_page_socket_url(String.t(), String.t(), non_neg_integer()) :: {:ok, String.t()} | {:error, String.t()}
  ...
  defp handle_decoded_frame(state, {:ok, %{"method" => method, "params" => params}})
       when is_binary(method) and is_map(params) do
    maybe_forward_page_event(state.owner, state.target_id, method, params)
    state
  end

  defp handle_decoded_frame(state, _decoded), do: state

  defp maybe_forward_page_event(owner, target_id, method, params)
       when is_pid(owner) and is_binary(target_id) and is_binary(method) and is_map(params) do
    send(owner, {:cerberus_cdp_page_event, target_id, method, params})
    :ok
  end

  defp maybe_forward_page_event(_owner, _target_id, _method, _params), do: :ok
  ...
  defp find_page_socket_url(targets, debugger_address, target_id, attempts_left)
    ...
    case Enum.find(targets, &match_target?(&1, target_id)) do
      ...
      _ ->
        retry_fetch_page_socket_url(debugger_address, target_id, attempts_left)
    end
  end

  defp retry_fetch_page_socket_url(debugger_address, target_id, attempts_left)
       when is_binary(debugger_address) and is_binary(target_id) and is_integer(attempts_left) and attempts_left > 0 do
